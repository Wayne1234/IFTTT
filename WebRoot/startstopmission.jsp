...
<div class="my">
<%rs = statement.executeQuery("select * from task where user='"+user+"' ");
rs.last(); 
...
mission : <select name="select" id="select_k1" class="xla_k">
<%
for(int i=1;i<=rowCount;i++) {//3������ 2��id
...
<p>start the mission<input type="radio" name="sors" value="start"></p>
<p>stop the mission<input type="radio" name="sors" value="stop"></p>
...
for(int k=1;k<=rowCount;k++) {
	%>
<table width="601" height="50" border="1" align="center" color="red">
<%
...
taskname: <%=l%>  type:<%=c%>  <br>
<%if(c.equals("type1"))
	{
	%>at time <%=d%>  <%=e%> send email to: <%=j%><%
	}
  if(c.equals("type2"))
	{
		%>at time <%=d%>  <%=e%> send weibo to: <%=h%><%
	}
  if(c.equals("type3"))
	{
	  %>send email to <%=j%> while <%=f%> receive an email<%
	}
	if(c.equals("type4"))
	{
		%>send weibo to <%=h%> while <%=f%> receive an email<%
	}
%>
</tr></table><br><%
}
